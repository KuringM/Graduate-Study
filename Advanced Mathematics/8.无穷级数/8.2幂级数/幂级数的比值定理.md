...
根据[[比值审敛法]],
那么当 $\rho|x|<1$ 即 $|x|<\frac{1}{\rho}$ 时, 正项级数收敛, 从而幂级数绝对收敛; <BR>
...
因此一般项 $\left|a_{n} x^{n}\right|$ 不能趋于零, 所以 $a_{n} x^{n}$ 也不能趋于零,
...
(3)
如果 $\rho=+\infty$, 那么对于除 $x=0$ 外的其他一切 $x$ 值, 根据[[比值审敛法]]和**级数收敛的必要条件**知幂级数必发散, <BR>
否则由[[阿贝尔定理]]知道将有点 $x \neq 0$ 使正项级数收敛.
于是 $R=0$.
