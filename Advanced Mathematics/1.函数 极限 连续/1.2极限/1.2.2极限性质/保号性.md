...
$$
\begin{aligned}
	& \mathrm{if\ in}\ \mathring{U} (x_0), f(x) \geqslant 0(\mathrm{or}\ f(x) \leqslant 0), \mathrm{and}\ \lim\limits_{x\to x_0} f(x) = A, \mathrm{then}\ A \geqslant (\mathrm{or}\ A\leqslant 0).
\end{aligned}
$$
